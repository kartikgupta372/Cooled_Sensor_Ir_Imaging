...
# ── Load or auto-generate calibration files ───────────────────────────────────
def load_calibration(calib_dir: str = "data/calibration_assets") -> dict:
    ...
    for key, fname in files.items():
        ...
        if os.path.exists(fpath):
            ...
            calib[key] = None   # will be estimated from frame
    return calib


# ── Per-frame calibration estimates (no pre-computed files needed) ────────────
...
def estimate_flat_field(frame: np.ndarray) -> np.ndarray:
    ...
    """
    import cv2
    # Very heavy blur to get only large-scale variation
    ksize = max(21, (min(frame.shape[:2]) // 8) | 1)   # must be odd
...
def run_calibration(frame: np.ndarray,
    ...
    # ── Linearity LUT (optional) ──────────────────────────────────────────────
    lut = calib.get("lut")
    if lut is not None:
        f = linearity_correction(f, lut)

    # ── Temperature mapping (optional) ────────────────────────────────────────
    if to_kelvin:
...
# ── Generate calibration files from a set of frames ───────────────────────────
