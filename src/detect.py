...
# ── IR-specific class labels (hotspot-based, no ML model needed) ──────────────
IR_CLASSES = {
...
# Override key classes with distinct, readable colours
CLASS_COLORS.update({
    "person":       (0,   255,  0),
    "car":          (255, 200,  0),
    "truck":        (255, 140,  0),
...
@dataclass
class Detection:
    bbox: tuple          # (x, y, w, h) — top-left + size
    ...
    source: str = "yolo" # "yolo" | "hotspot" | "thermal"
    extra: dict = field(default_factory=dict)


# ══════════════════════════════════════════════════════════════════════════════
# YOLO DETECTOR — 80 COCO classes, works on any image
# ══════════════════════════════════════════════════════════════════════════════
class UniversalYOLODetector:
    ...
    def _prepare_image(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Grayscale → 3-channel (YOLO needs 3 channels)
        if img.ndim == 2:
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                # Sample mean intensity from detection region
...
# ══════════════════════════════════════════════════════════════════════════════
# THERMAL HOTSPOT DETECTOR — IR / thermal specific, no model needed
# ══════════════════════════════════════════════════════════════════════════════
class ThermalHotspotDetector:
    ...
    def _threshold_detect(self, gray: np.ndarray, percentile: float,
        ...
        # Calculate global stats to avoid detecting noise in flat images
        ...
        # Critical fix: Don't just take top 10% of empty background noise.
        # It must be actually bright/cold relative to the whole image.
        if from_top:
        ...
        for cnt in contours:
            ...
            # Confidence based on how extreme the region is
...
# ══════════════════════════════════════════════════════════════════════════════
# UNIFIED DETECTOR — combines YOLO + thermal + tracking
# ══════════════════════════════════════════════════════════════════════════════
class UnifiedDetector:
    ...
    def detect(self, frame: np.ndarray,
        ...
        # Thermal hotspot detection on grayscale channel
        if self.thermal and is_thermal:
            thermal_dets = self.thermal.detect(frame)
            # Only add thermal detections that don't overlap with YOLO
